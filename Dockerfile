...
RUN pnpm config set store-dir /pnpm/store && pnpm fetch --frozen-lockfile && pnpm install --offline --frozen-lockfile
...
RUN apk add --no-cache ca-certificates su-exec tzdata gettext wget && \
